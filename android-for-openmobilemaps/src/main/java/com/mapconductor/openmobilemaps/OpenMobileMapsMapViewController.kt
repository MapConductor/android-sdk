...
class OpenMobileMapsMapViewController(
    ...
    private fun notifyCamera(camera: OpenMobileMapsCamera) {
     ...
     */
    fun readNativeCamera(camera: OpenMobileMapsCamera = holder.map.camera): MapCameraPosition =
        MapCameraPosition(
    ...
    fun declareCapabilities(registry: MutableMapServiceRegistry) {
        ...
        registry.declareUnsupported(
            ...
            "the openmobilemaps map renders a flat web-mercator surface and has no tilt",
        )
        registry.declareUnsupported(
            MapCapability.CameraRotate,
            "the openmobilemaps map renders a north-up surface and has no bearing",
        )
    }
