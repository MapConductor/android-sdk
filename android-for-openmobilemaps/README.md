...
### 2-6. タッチ座標は「内側ビューの座標系」で届く

Android は子ビューへの MotionEvent を**子のローカル座標へ変換してから**渡す。
SDK のタッチハンドラにも、内側ビューの `OnTouchListener` にも、届く座標は
すでに内側 `MapView` の座標系である。そこへ `fromScreenOffsetSync`
...
タップに反応しなくなる**という形で発覚した。タッチ経路は必ず
`fromInnerOffsetSync` を使うこと。

### 2-7. 空タイルの 404 はエラー扱いになり、マスクに穴が開く

マーカータイルのローカルサーバは、マーカーが無いタイルに 404 を返す
（MapLibre はそれを空タイルとして扱う）。Open Mobile Maps は 404 を**エラー**として
記録し、そのタイルを描かない。子タイルが無い領域は 2-8 のステンシルマスクに
**穴が開き、粗い親タイルがそこだけ透けて見え続ける**（マーカーの無い領域だけ
巨大な赤い矩形が残る、という形で出た）。

`DataLoader` の 204 = 空タイル分岐は `isSuccessful && bytes != null` の**後ろ**に
あるため実質届かない。対策はインターセプタでローカルサーバの 404 を
**「透明な 1x1 PNG の 200」へ書き換える**こと（`emptyLocalTileInterceptor`）。

### 2-8. 透過ラスターレイヤは `maskTile = true` にしないと粗い親タイルが透けて残る
