...
## 5. 未解決: PostOffice ページでマーカータイルが巨大かつぼやける

ズームインしていくと、マーカータイルのアイコンが**巨大でぼやけたまま画面を覆い**、
放置しても直らない。**原因はまだ特定できていない。** 次に見る人のために、
分かっていることと**潰した仮説**を残す。

### 分かっていること（すべて実機で計測）

- ラスターレイヤは**作り直されていない**。`createLayer` はセッション中 1 回だけ。
  地図上のレイヤ数も想定どおりで、古いレイヤが残っているわけでもない
  （「何度も読み込まれる」という見え方だが、レイヤの再生成ではない）
- タイルは**正しいレベルで要求されている**。統一ズーム 10 でレベル 11、
  ズームインに追従して 12→17 と進む。地図デザインのレイヤと同じ進み方
- **現在レベルのタイルは正しい**。`numDrawPreviousLayers = 0` にすると、
  正しい大きさの鮮明なアイコンが、ぼやけた巨大な層の**上に**現れる
- つまり残っているのは**粗いレベルのタイル**で、それが画面全体を覆っている。
  PostOffice はマーカーが密でタイルがほぼ一色の赤になるため、覆われると全面が赤くなる
- 20 秒放置してもタイル要求は 0 件のまま、見た目も変わらない（読み込み待ちではない）
- 地図デザインのレイヤでは起きない（細かいタイルが同じ絵で上書きするので目立たないだけ、
  という可能性はある）

### 潰した仮説

- タイル URL の引数順（`getTileUrl(x, y, t, zoom)` で正しい）
- レベル選択への端末密度の混入（**実際に間違っていたので直したが、症状は変わらない**）
- `overzoom` / `underzoom` / `numDrawPreviousLayers` / `maskTile` の組み合わせ
  （すべて切っても粗い層は消えない）
- ラスターレイヤの多重生成、古いレイヤの外し漏れ

### 次に見るところ

粗いレベルのタイルが `Tiled2dMapSource` の中で「現在のレベル」として保持され続けて
いるように見える。`Tiled2dMapZoomInfo.zoomLevelScaleFactor` は 1.0 にしているが、
SDK 同梱の web メルカトル設定は 0.65 を使っている。レベルの切り替え判定の
しきい値なので、ここが効いている可能性がある。
