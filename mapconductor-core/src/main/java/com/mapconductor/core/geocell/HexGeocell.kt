...
data class HexCoord(val q: Int, val r: Int, val depth: Int = 0) {
    override fun toString(): String {
        return "H${q}_${r}_${depth}"
    }
}
enum class Direction6(val dq: Int, val dr: Int) {
    Right(1, 0),
    RightUp(1, -1),
    LeftUp(0, -1),
    Left(-1, 0),
...
data class HexCell(
    ...
    fun idPrefix(levels: Int): String {
        return id.split("_").take(levels + 1).joinToString("_")
    }
}
...
class HexGeocell(
    ...
    fun enclosingCellOf(points: List<IdentifiedPoint>, zoom: Double): HexCell {
    ...
    fun hexCellsForPointsWithId(points: List<IdentifiedPoint>, zoom: Double): Set<IdentifiedHexCell> {
        return points.map {
