 ...
 */
class MarkerSpatialService : Service() {
    
    companion object {
        private const val TAG = "MarkerSpatialService"
    ...
    private val binder = object : android.os.Binder() { // Temporarily disabled: IMarkerSpatialService.Stub() {
        
        fun initializeSession(sessionId: String, config: SpatialConfigDTO): Boolean {
            return try {
        ...
        fun updateMarkers(sessionId: String, markers: List<MarkerDataDTO>): Boolean {
            return try {
                val session = sessions[sessionId] ?: return false
                
                // Throttle markers if too many
                val limitedMarkers = if (markers.size > MAX_MARKERS_PER_SESSION) {
                ...
                val markerStates = limitedMarkers.map { dto ->
                    MarkerState(
                        id = dto.id,
                ...
                runBlocking {
                    markerStates.forEach { state ->
                        val entity = MarkerEntityImpl<String>(
                            state = state,
        ...
        fun removeMarkers(sessionId: String, markerIds: List<String>): Boolean {
            return try {
                val session = sessions[sessionId] ?: return false
        ...
        fun calculateSpatialChanges(sessionId: String, camera: CameraPositionDTO): SpatialResultDTO {
            return try {
                ...
                if (!session.config.addOnlyMode) {
                    val markersToRemoveSet = session.renderedMarkers.filter { id ->
                ...
                Log.d(TAG, "Spatial calculation for $sessionId: +${markersToAdd.size} -${markersToRemove.size} ~${markersToUpdate.size}")
        ...
        fun findNearestMarker(sessionId: String, latitude: Double, longitude: Double): String? {
            return try {
    ...
    override fun onBind(intent: Intent?): IBinder {
        Log.d(TAG, "MarkerSpatialService bound with intent: $intent")
    ...
    override fun onCreate() {
        super.onCreate()
        Log.d(TAG, "MarkerSpatialService created in process: ${android.os.Process.myPid()}")
    }
    ...
    override fun onDestroy() {
        ...
        Log.d(TAG, "MarkerSpatialService destroyed")
    }
}