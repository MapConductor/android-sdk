...
class RemoteSpatialMarkerRenderingStrategy<ActualMarker>(
    ...
    init {
        ...
        strategyId = MarkerServiceManager.registerStrategy(context, this)
        connectToService()
        startBatchProcessor()
    }
    ...
    override suspend fun onCameraChanged(
        ...
        semaphore.withPermit {
            try {
                ...
                val expandedBounds = com.mapconductor.core.spherical.expandBounds(visibleRegion.bounds, expandMargin)
                ...
                val currentlyRendered = allEntities
    ...
    override suspend fun onAdd(
        ...
        return try {
            ...
            Log.d(TAG, "Added ${data.size} markers (${markersToRender.size} rendered immediately) to session $sessionId")
            true
        } catch (e: Exception) {
        ...
        return try {
            ...
            if (isInViewport && !wasRendered) {
                ...
                actualMarkers.firstOrNull()?.let { actualMarker ->
                    val updatedEntity = MarkerEntityImpl<ActualMarker>(
                        state = state,
                        marker = actualMarker,
                        isRendered = true
                    )
                ...
                val changeParams = object : MarkerOverlayRenderer.ChangeParams<ActualMarker> {
                    override val current = MarkerEntityImpl(
                        state = state,
     ...
     */
    fun findNearestMarker(latitude: Double, longitude: Double): MarkerEntity<ActualMarker>? {
        return try {
            if (!waitForServiceConnection()) return null
            
            // Using local fallback for nearest marker
            markerManager.findNearest(com.mapconductor.core.features.GeoPoint.fromLatLong(latitude, longitude))
        } catch (e: Exception) {
    ...
    fun destroy() {
        try {
            ...
            MarkerServiceManager.unregisterStrategy(context, strategyId)
