 ...
 */
fun calculateScaleFromZoomLevel(
    zoomLevel: Double,
    latitude: Double = 0.0,
    dpi: Double = 96.0,
): Double {
 ...
 */
fun calculateZoomLevelFromScale(
    scale: Double,
    latitude: Double = 0.0,
    dpi: Double = 96.0,
): Double {
...
private const val DEFAULT_MAX_GMAPS_TILT = 60.0 // Google Maps Tilt の想定最大値
...
// --- 以前の角度変換関数 ---
fun arcgisPitchToGoogleMapsTilt(
    pitch: Double,
    maxTilt: Double = DEFAULT_MAX_GMAPS_TILT,
): Double = pitch.coerceIn(MIN_ANGLE, maxTilt)

fun googleMapsTiltToArcgisPitch(tilt: Double): Double = tilt.coerceIn(MIN_ANGLE, ARCGIS_MAX_PITCH)
