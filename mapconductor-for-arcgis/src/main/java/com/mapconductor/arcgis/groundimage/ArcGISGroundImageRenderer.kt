...
) : AbstractGroundImageRenderer<ArcGISActualGroundImage>() {
    override suspend fun addGroundImages(newGroundImages: List<GroundImageState>): List<ArcGISActualGroundImage?> =
        emptyList()

    override suspend fun changeGroundImages(
        changes: List<GroundImageRenderer.UpdateParams<ArcGISActualGroundImage>>,
    ): List<Graphic?> = emptyList()

    override suspend fun removeGroundImages(removeEntities: List<GroundImageEntity<ArcGISActualGroundImage>>) {
    }
}
