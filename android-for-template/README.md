...
| # | 実装点 | 個数 | ファイル |
|---|---|---|---|
| A | ホルダー — `mapView` / `map` / 投影 2 つ | 4 | `TemplateMap.kt` |
| B | コントローラ — `holder` / カメラ読み書き / `fitBounds` | 5 | `TemplateMapViewController.kt` |
| C | 地図デザイン型 | 3 | `TemplateMapView.kt` |
| D | **レンダラ 6 種 × onAdd/onChange/onRemove/onPostProcess** | 25 | `TemplateOverlays.kt` |
| E | SDK イベントの転送 | 6 | `installListeners()` |
| F | ドラッグ中のパン抑止 | 1 | SDK 次第 |
| G | State サブクラス | 3 | `TemplateMapView.kt` |
...
1. `android-for-template` をコピーして `android-for-<sdk>` にする
2. `TemplateMap.kt` の `TemplateMap` を実際の SDK の地図型に置き換える
3. `TemplateOverlays.kt` の 6 レンダラを SDK のオブジェクト生成に書き換える（**ここが本体**）
4. `TemplateMapViewController.kt` のカメラ換算とイベント転送を SDK に合わせる
5. `TemplateMapView.kt` の Canvas を `AndroidView { SDKのMapView }` に置き換える
6. `TemplateDriverConformanceTest.kt` をそのまま動かす
7. **実機で確かめる**（§6）
...
`TemplateDriverConformanceTest.kt` に実例があります。素の JUnit で動きます
