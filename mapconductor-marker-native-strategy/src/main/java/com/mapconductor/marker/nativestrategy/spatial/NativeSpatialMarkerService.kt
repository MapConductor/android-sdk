 ...
 */
class NativeSpatialMarkerService : Service() {
    companion object {
    ...
    private val binder = object : INativeSpatialMarkerService.Stub() {
        override fun initializeSession(
            sessionId: String,
            config: NativeSpatialConfigDTO,
        ): Boolean = try {
            Log.d(TAG, "initializeSession: $sessionId")
            val strategy = NativeRemoteSpatialEngine.create(config.expandMargin, config.addOnlyMode)
                ?: return false
        ...
        ): NativeSpatialResultDTO = try {
            val strategy = sessions[sessionId] ?: return NativeSpatialResultDTO()
            val result = strategy.processCameraChange(
                CameraPosition(
        ...
        override fun destroySession(sessionId: String): Boolean = try {
        ...
        override fun getPerformanceStats(sessionId: String): String = try {
