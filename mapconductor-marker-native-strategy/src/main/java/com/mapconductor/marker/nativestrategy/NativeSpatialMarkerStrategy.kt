 ...
 * @param addOnlyMode If true, markers are never removed once rendered (like NativeAddOnlyMarkerStrategy)
 * @param semaphore Semaphore for synchronizing rendering operations
 */
class NativeSpatialMarkerStrategy<ActualMarker>(
    ...
    override suspend fun onCameraChanged(
        ...
        semaphore.withPermit {
            ...
            if (markersInBounds.isEmpty() && allEntities.isNotEmpty()) {
                // Native spatial query likely failed, use fallback approach like SpatialMarkerStrategy
...
object NativeSpatialMarkerRenderingStrategies {
    ...
    ): NativeSpatialMarkerStrategy<ActualMarker> =
