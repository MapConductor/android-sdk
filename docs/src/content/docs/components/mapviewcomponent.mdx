...
description: Provider-specific Jetpack Compose map views for Google Maps, Mapbox, HERE, ArcGIS, and MapLibre in MapConductor Android SDK.
---
...
MapConductor provides a MapViewContainer component that wraps the MapView provided by the map SDK.
...
![MapViewContainer Architecture](~/assets/mapviewcomponent/architecture.png)
...
map and camera callbacks are set on the map provider-specific component, while overlay callbacks are set on each State.
